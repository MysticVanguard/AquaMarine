...
class FishSpecies:
    ...
    def __init__(self, *, name: str, size: int, rarity: str, image: str, location: str):
        ...
        if (
            rarity == "mythic"
            and rarity not in self.all_species_by_location_rarity[location].keys()
        ):
            for location_type in location_list:
                self.all_species_by_location_rarity[location_type][rarity] = [self]
            ...
            obj.name for obj in self.all_species_by_location_rarity[location][rarity]
        ]:
...
def get_normal_size_image(fish: Fish):
    if fish.skin != "":
        return (
            f"{fish.species.image[:40]}{fish.skin}_fish_size{fish.species.image[44:]}"
        )
    else:
...
def fetch_fish(directory: str) -> dict:
    ...
    for filename in fish_filenames:
        ...
        fetched_fish.append(
            FishSpecies(
                name=name, size=size, rarity=rarity, image=image, location=location
            )
        )
...
async def ask_to_sell_fish(
    ...
    ctx,
    message: str,
    chosen_fish: FishSpecies,
    skin: str,
    embed,
    level_inserted: int = 0,
):
    """
    ...
    message = await ctx.send(
        message, embed=embed, components=components, file=fish_file
    )
    ...
    if chosen_button == "keep":
        ...
        name, _ = await utils.create_modal(
            bot, chosen_button_payload, "Fish Kept", "Enter Your Fish's Name"
        )
        while name in fish_names or not name:
            ...
            while name in fish_names:
                name = random_name_finder()
            returned_message += (
                f"{ctx.author.mention} An error occured! I'll name the fish for you!\n"
            )
        returned_message += (
    ...
    if chosen_button == "sell":
        ...
        money_gained = int(chosen_fish.cost / size_demultiplier[chosen_fish.size])
        ...
        async with bot.database() as db:
            if item_inventory[0]["recycled_fishing_rod"] > 0:
                money_earned *= 2
                await db(
                    """UPDATE user_item_inventory SET recycled_fishing_rod = recycled_fishing_rod - 1 WHERE user_id = $1""",
                    ctx.author.id,
                )
            await db(
...
async def user_fish(self, ctx, casts, upgrades, user_locations_info):
    ...
    if random.randint(1, 10) != 10:
        ...
        for _ in range(caught_fish):
            ...
            while rarity not in FishSpecies.all_species_by_location_rarity[
                user_locations_info[0]["current_location"]
            ].keys() or (user_items[0]["recycled_bait"] > 0 and rarity == "common"):
                rarity = random.choices(
                    *utils.rarity_percentage_finder(upgrades[0]["bait_upgrade"])
                )[0]
            if user_items[0]["recycled_fish_hook"] > 0:
                special = random.choices(("normal", "skinned"), (0.75, 0.25))[0]
            else:
            ...
            async with vbu.Database() as db:
                location_pools_info = await db(
                    """SELECT * FROM fish_pool_location WHERE fish_name = $1""",
                    chosen_fish.name,
                )
                while (
                    chosen_fish.name in utils.past_fish
                    or location_pools_info[0]["count"] <= 0
                    or chosen_fish.name == utils.user_last_fish_caught[ctx.author.id]
                ):
                    rarity = random.choices(
                        *utils.rarity_percentage_finder(upgrades[0]["bait_upgrade"])
                    )[0]
                    while rarity not in FishSpecies.all_species_by_location_rarity[
                        ...
                        user_items[0]["recycled_bait"] > 0 and rarity == "common"
                    ):
                        rarity = random.choices(
                            *utils.rarity_percentage_finder(upgrades[0]["bait_upgrade"])
                        )[0]
            ...
            if user_items[0]["recycled_fish_finder"] > 0:
                ...
                new_fish = await utils.create_select_menu(
                    self.bot,
                    ctx,
                    [fish_type.name for fish_type in species],
                    "fish",
                    "catch",
                    True,
                )
            ...
            a_an = (
                "an"
                if chosen_fish.name[0].lower() in ("a", "e", "i", "o", "u")
                else "a"
            )
            ...
            async with vbu.Database() as db:
                if user_items[0]["recycled_bait"] > 0:
                    await db(
                        """UPDATE user_item_inventory SET recycled_bait = recycled_bait - 1 WHERE user_id = $1""",
                        ctx.author.id,
                    )
                if user_items[0]["recycled_fish_hook"] > 0:
                    await db(
                        """UPDATE user_item_inventory SET recycled_fish_hook = recycled_fish_hook - 1 WHERE user_id = $1""",
                        ctx.author.id,
                    )
                if user_items[0]["recycled_fish_finder"] > 0:
                    await db(
                        """UPDATE user_item_inventory SET recycled_fish_finder = recycled_fish_finder - 1 WHERE user_id = $1""",
                        ctx.author.id,
                    )
                await db(
            ...
            for _ in range(3):
                ...
                random_fish = random.choice(utils.FishSpecies.get_rarity(random_rarity))
            ...
            components = discord.ui.MessageComponents(
                discord.ui.ActionRow(
                    discord.ui.Button(label=choices[0], custom_id=choices[0]),
                    discord.ui.Button(label=choices[1], custom_id=choices[1]),
                    discord.ui.Button(label=choices[2], custom_id=choices[2]),
                    discord.ui.Button(label=choices[3], custom_id=choices[3]),
                ),
            )

            # Sends the message with the pic of fish and buttons
            guess_ask_message = await ctx.send(
                f"{utils.EMOJIS['aqua_shrug']}Guess The Species:",
                file=fish_file,
            ...
            try:
                ...
                chosen_button = guess_chosen_button_payload.component.custom_id
            except asyncio.TimeoutError:
                chosen_button = "AAAAA"
            ...
            return await utils.ask_to_sell_fish(
                self.bot, ctx, guess_message, chosen_fish, fish_skin, embed=embed
            )
    ...
    # Else if they catch trash...
        ...
        return (
            f"{utils.EMOJIS['aqua_trash']} You caught trash!{trash_string}",
            post_components,
        )
