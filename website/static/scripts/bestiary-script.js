async function getNames(data) {
    let site = await fetch("/fish_names", {
        method: "POST",
        headers: {
...
async function autocomplete(inp) {
    ...
    inp.addEventListener("input", function (e) {
        ...
        for (i = 0; i < arr.length; i++) {
            ...
            if (arr[i].substr(0, val.length).toUpperCase() == val.toUpperCase() && a.children.length < 5) {
...
async function showAllCategoriesOnClick(inp) {

    let response = await getNames(1);
    let arr = response.names;
    function closeAllLists(elmnt) {
        /*close all autocomplete lists in the document,
        except the one passed as an argument:*/
